...
如果在较深目录安装 PySide6 时遇到 Windows 长路径限制，可以把依赖安装到短路径，运行测试或启动 GUI 时通过 `PYTHONPATH` 引入。例如：

```powershell
.\.venv\Scripts\python.exe -m pip install --target C:\Users\LBJ\Documents\Codex\pyside6-py312 PySide6
$env:PYTHONPATH="C:\Users\LBJ\Documents\Codex\pyside6-py312"
```
...
## 基础输出
- `output/full_concat.mp4`：按最终顺序拼接的归档视频。
