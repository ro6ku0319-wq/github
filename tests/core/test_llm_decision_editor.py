...
def test_llm_decision_editor_round_trips_editable_segment_fields(tmp_path: Path) -> None:
    ...
    rows[0]["symmetry_group"] = "front_blockout_pair_01"
    rows[0]["symmetry_side"] = "left"
    rows[0]["symmetry_keep_role"] = "representative"
    ...
    assert updated["segments"][0]["symmetry_group"] == "front_blockout_pair_01"
    assert updated["segments"][0]["symmetry_side"] == "left"
    assert updated["segments"][0]["symmetry_keep_role"] == "representative"
